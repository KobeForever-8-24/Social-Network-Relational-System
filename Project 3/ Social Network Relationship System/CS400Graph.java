 ...
 * @description: The class that implementing the Dijkstra's Shortest Path Algorithm
 * @author: Weijie Zhou
...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        // Check if these two vertices contain in the graph
        if(!containsVertex(start) || !containsVertex(end)){
            throw new NoSuchElementException("There is no vertex containing input data");
        }

        // Create a path to store the start vertex
        Path returnPath = new Path(vertices.get(start));

        // Create a priority queue to store any possible edge from vertex
        PriorityQueue<Path> queue = new PriorityQueue<Path>();

        // Add the return Path to the queue
        queue.add(returnPath);

        // Do a while until reach the end vertex
        while (queue.peek().end.data != end){

            // Store vertices which have already been visited
            List<T> visitedNode = new LinkedList<T>();

            // Find the new vertex data and add into the visitedNode list
            for(T data : queue.peek().dataSequence){
                if(!visitedNode.contains(data))
                    visitedNode.add(data);
            }

            // Use a for loop to go through all edges of the new vertex
            for(Edge edge : queue.peek().end.edgesLeaving){
                // Create a temporary path to store all possible edges
                Path tempPath = new Path(queue.peek());
                // Use the edge to extend the path
                tempPath.extend(edge);

                // If the end of vertex of the temporary path had been visited, do not add to the queue
                if(visitedNode.contains(tempPath.end.data))
                    continue;

                queue.add(tempPath);
            }

            // Remove previous old path
            queue.poll();

            if(queue.size() == 0)
                throw new NoSuchElementException("There is no such path from start vertex to end vertex");
        }

        // Get the path from the queue
        returnPath = queue.poll();
        return returnPath;
